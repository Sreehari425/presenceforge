...
All three runtimes provide the same API, making it easy to switch between them.
...
async fn main() -> Result {
    // Create client
    let mut client = new_discord_ipc_client("your_client_id").await?;
...
2. Change `DiscordIpcClient::new()` to `new_discord_ipc_client().await`
